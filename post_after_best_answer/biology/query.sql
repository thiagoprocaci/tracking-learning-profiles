...
select -- accepted.id, 
		...
		u.profile_1 ,
	 	u.profile_2
 from biology_answer accepted 
inner join biology_answer afterAccepted on afterAccepted.parent_post_comm_id = accepted.parent_post_comm_id
inner join biology_user_profile u on afterAccepted.id_user = u.id
...
select B.question_id ,
	...
	select A.question_id, A.answer_type, count(*) as answers_count from (
		...
		from biology_question q 
			...
			inner join biology_answer best on best.id_post_comm = q.accepted_answer_comm_id
